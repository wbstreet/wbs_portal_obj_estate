...
if ($action == 'edit') {
    ...
    if (isset($_FILES['photos']['tmp_name'])) {
        ...
        //print_error(json_encode($_FILES));
    }
    ...
    if ($apartment_id === null) {
        ...
        if ($ids) $clsModPortalObjEstate->add_image($apartment_id, $ids);

            print_success('Объявление успешно добавлено! '.$img_errs, ['absent_fields'=>[]]);
        ...
        if ($ids) {
            $r = $clsModPortalObjEstate->add_image($apartment_id, $ids);
            if ($r !== true) return $r;
        }
